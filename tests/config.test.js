...
describe("resolveConfig — defaults", () => {
  it("fills in every default key", () => {
    ...
    expect(c.contentDir).toBe(DEFAULTS.contentDir);
    expect(c.linkPrefix).toBe(DEFAULTS.linkPrefix);
    expect(c.prerender).toBe(true);
  ...
  it("preserves explicit prerender: false", () => {
    const c = resolveConfig({ site: "https://example.com", prerender: false });
    expect(c.prerender).toBe(false);
  });
...
describe("resolveConfig — validation", () => {
  ...
  it("throws when prerender is not a boolean", () => {
    expect(() =>
      resolveConfig({ site: "https://x.com", prerender: "yes" }),
    ).toThrow(/prerender must be a boolean/);
  });

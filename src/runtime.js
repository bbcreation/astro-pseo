 ...
 */
export function buildArticleHtml(raw) {
 ...
 */
/**
 * Build a pagination nav HTML fragment. Page 1 lives at `linkPrefix`;
 * pages 2+ at `${linkPrefix}/p/<n>`. Renders prev / page-window (5
...
export function buildPaginationHtml({ currentPage, totalPages, linkPrefix }) {
  ...
  for (let n = from; n <= to; n++) {
    middle += n === currentPage
      ? `<span class="pseo-current">${n}</span>`
      : `<a href="${href(n)}">${n}</a>`;
  }

  return `<nav class="pseo-pagination" aria-label="Pagination">${prev}${middle}${next}</nav>`;
}

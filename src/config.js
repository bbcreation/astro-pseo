 ...
 * @property {number}  [perPage]          Articles per page (SSR only). Default 15. Ignored when prerender:true.
 * @property {boolean} [prerender]        Static prerender mode. Default true. Set false for v1 SSR behavior.
...
export function resolveConfig(user) {
  ...
  const merged = {
    ...
    perPage: u.perPage ?? DEFAULTS.perPage,
    prerender: u.prerender ?? DEFAULTS.prerender,
  };
...
function validate(c) {
  ...
  if (typeof c.prerender !== "boolean") {
    throw new Error("[astro-pseo] config.prerender must be a boolean.");
  }

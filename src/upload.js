...
export async function handleUploadPost(context, config) {
  ...
  try {
    ...
    return redirect(basePath, "err=1&msg=Invalid+request+payload.");
  }
  ...
  if (action === "upload") {
    ...
    if (typeof file.size === "number" && file.size > MAX_UPLOAD_BYTES) {
      return redirect(basePath, "err=1&msg=File+too+large+(max+50+MB).");
    }
...
function renderPage({ authenticated, files, flash, isError }) {
  ...
  const noticeHtml = flash
    ? `<div class="notice ${isError ? "notice-err" : "notice-ok"}"><pre>${escHtml(decodeURIComponent(flash.replace(/\+/g, " ")))}</pre></div>`
    : "";
      ...
      <div class="file-row">
        ...
        <div class="file-btns">
          <form method="POST" style="display:contents">
            ...
            <input type="hidden" name="filename" value="${escHtml(f)}">
            <button type="submit" class="btn btn-sm">Import</button>
          </form>
          <form method="POST" style="display:contents"
                onsubmit="return confirm('Delete ${escHtml(f)}?')">
